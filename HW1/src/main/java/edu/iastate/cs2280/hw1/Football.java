...
/**
 * The Football Class in one of 7 different Household preferences that some household in a NeighborhoodGrid
 * could have. The Football class contains constructors to initialize Football objects, primarily in a
...
public class Football extends SportsHouseholds{
    ...
    /**
     * Parameterized Constructor: Constructs a Football object in specified NeighborhoodGrid object "grid"
     * at position "row", "column", with an interest level of "interestLevel".
     *
     * @param grid The Neighborhood object whose grid should contain the newly created Football household
     ...
     * @param interestLevel The level of interest the household has for Football
     */
    ...
    /**
     * Overridden Method: Returns the Constant "FOOTBALL" from the Sports Enum to determine preference
    ...
    /**
     * Overridden Method: Returns the string representation of the Football Household, which is the Household's
     * Sport Preference ("F" for Football) followed by its interest level and one whitespace.
     */
    @Override
    public String toString() {
        return "F" + interestLevel + " ";
    }

    /**
     * Overridden Method: Determines the state of the Football household in the next simulation step based on its
    ...
    public Household next(NeighborhoodGrid newGrid, int month) {
        ...
        //If months of simulation are either 4, 5, or 6
        if(month == 4 || month == 5 || month == 6) {
            //If (Everything >= 1) OR (Nothing >= 1)
            if (Household.members[2] >= 1 || Household.members[4] >= 1) {

                //If interest lvl is not at max, increments lvl by one
                if (this.getInterest() < MAX_INTEREST) {
                    //Updates newGrid with proper household, and then returns such household
                    newGrid.grid[row][column] = new Football(neighborhoodGrid, this.row, this.column,
                    this.interestLevel + 1);
                    return newGrid.grid[row][column];

                //Otherwise, household stays as it is at lvl 5
                } else {
                    //Updates newGrid with proper household, and then returns such household
        ...
        //If (Baseball + Basketball + Football > 7), then household becomes an Everything household
        } else if(Household.members[0] + Household.members[1] + Household.members[3] > 7){
            //Updates newGrid with proper household, and then returns such household
        ...
        //If (Football < 2), then household becomes a Baseball household, lvl 0.
        } else if(Household.members[3] < 2){
            //Updates newGrid with proper household, and then returns such household
            newGrid.grid[row][column] = new Baseball(neighborhoodGrid, this.row, this.column, 0);
            return newGrid.grid[row][column];

        //Else, household becomes a Football household with lvl incremented by 1
        } else {
            //Updates newGrid with proper household, and then returns such household
            newGrid.grid[row][column] = new Football(neighborhoodGrid, this.row, this.column,
            this.interestLevel + 1);
